...
![An illustration of a layout shift](./layout_shift.gif)

This data is provided by an API,
so you trigger an HTTP request to get the view count,
and then display it. 
Sadly, the loading delay creates a layout shift,
...
A totally empty span has no height.
...
## 🔨 Practice: a "NoShiftClientOnlySpan" component

import SkipPractice from "@/components/SkipPractice.mdx"

<SkipPractice />

- Open the component named `NoShiftClientOnlySpan`
- If `hasData` is false, render an invisible span: `<span style={{ opacity: 0 }}>{"| "}</span>`
- If `hasData` is true, return a span containing the children prop
...
The `useClient` hook can also be used 
if you need to use the `window` object
or any client-side only API 
...
after a server-render. There are a few different ways to achieve that in Next.js :

1) **Get the visitor count from a React Server Component.**

Keep in mind that fetching dynamic data like a view count
may make the whole page dynamically rendered, for every request.

2) **Get the visitor count from a React Server Component 
...
3) **Get the visitor count from a React Server Component and enable Partial Prerendering (PPR).**

The visitor count will be rendered dynamically on every request, 
...
4) **Keep your data fetching call client-side. It's fine!**

Don't get peer-pressured into using a modern pattern like server-side rendering,
if you don't need it in your application. Whatever works is fine :)
...
[Blazing Fast Next.js with React Server Components - Newline.co](https://www.newline.co/courses/blazing-fast-next.js-with-react-server-components)
